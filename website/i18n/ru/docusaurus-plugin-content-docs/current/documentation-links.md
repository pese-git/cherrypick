---
sidebar_position: 8
---

# Ссылки на документацию

Подборка внешних ресурсов по CherryPick.

## Пакеты на pub.dev

- [cherrypick](https://pub.dev/packages/cherrypick) — ядро DI (runtime)
- [cherrypick_annotations](https://pub.dev/packages/cherrypick_annotations) — аннотации DI
- [cherrypick_generator](https://pub.dev/packages/cherrypick_generator) — кодогенерация
- [cherrypick_flutter](https://pub.dev/packages/cherrypick_flutter) — интеграция с Flutter
- [talker_cherrypick_logger](https://pub.dev/packages/talker_cherrypick_logger) — логгер DI-событий на базе Talker
- [cherrypick_lint](https://github.com/pese-git/cherrypick/tree/master/cherrypick_lint) — lint-плагин для IDE, ловит неверное использование API (ещё не опубликован на pub.dev)

## Расширенные руководства в репозитории

- [Полный туториал](https://github.com/pese-git/cherrypick/blob/master/doc/full_tutorial_ru.md)
- [Аннотации и кодогенерация](https://github.com/pese-git/cherrypick/blob/master/doc/annotations_ru.md)
- [Обнаружение циклических зависимостей](https://github.com/pese-git/cherrypick/blob/master/doc/cycle_detection.ru.md)
- [Миграция на 4.0.0](https://github.com/pese-git/cherrypick/blob/master/doc/migration_4.0.0_ru.md)
- [README lint-плагина](https://github.com/pese-git/cherrypick/blob/master/cherrypick_lint/README.md) · [Гайд по линтингу](/ru/docs/linting)

## Исходный код

- [Репозиторий на GitHub](https://github.com/pese-git/cherrypick)
- [Список изменений](https://github.com/pese-git/cherrypick/blob/master/CHANGELOG.md)
