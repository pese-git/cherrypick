...
Опубликованные числа сняты на Dart 3.10.1 (stable), macOS arm64, локальная
...
Ожидается 90 зелёных тестов. Это не формальность — пять из них защищают
...
- `first_resolve_purity_test.dart` — подготовка хендлов не резолвит измеряемую
  цель до секундомера: первый `run()` обязан строить экземпляры сам, а не
  читать кеш, наполненный валидацией адаптера.
...
> **Историческая таблица.** Сравнение снято на первой редакции адаптеров
> (сентябрь 2026, до переписывания моста yx_scope и индекса riverpod) и
> приводится как пример метода — каких расхождений ожидать между прогонами.
> Числа «опубликовано» не совпадают с текущим REPORT_v2.md: адаптеры и SDK
> с тех пор менялись. Правило про разброс действует без изменений.
