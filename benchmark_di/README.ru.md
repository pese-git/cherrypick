...
> [METHODOLOGY.md](METHODOLOGY.md). Каждое решение там объяснено вместе с
> измерением, которое его вызвало. Несколько из них выглядят как лишнее
> усложнение — пачки замеров, отдельный процесс на сценарий, медиана вместо
> среднего, отказ контейнера от сценария вместо тихой подмены — и были бы
> откачены без этого документа.

> **Воспроизведение замеров:** [REPRODUCE.md](REPRODUCE.md) — точные команды за
> каждой таблицей отчётов, ожидаемый разброс между прогонами и что проверять,
> если ваши числа расходятся.
...
этих полей считается недействительным: JIT и AOT переставляют контейнеры
местами, а фаст-путь cherrypick работает только при выключенном детекторе
циклов.
...
> Короткие флаги пишутся через пробел (`-c 100`), не через знак равенства (`-c=100`). При `-c=100` значение приходит как `=100`, и прогон завершается кодом 64 вместо печати пустой таблицы.
