...
Эти методы await-ят `dispose()` для каждого `Disposable`, созданного биндингом, **объявленным в закрываемом скоупе**, обеспечивая корректную очистку (sync и async) и высвобождение ресурсов.

**Владение:** резолв через подскоуп не передаёт владение. Если биндинг объявлен
в root-скоупе, его экземпляры освобождаются при закрытии *root*, даже если были
получены через `subScope.resolve<T>()`. Чтобы привязать ресурс ко времени жизни
подскоупа, объявляйте его биндинг в этом подскоупе.

Вызывайте `await scope.dispose()` если вы явно управляете custom-скоупом.

> 💡 [`cherrypick_lint`](../cherrypick_lint) подсвечивает пропущенный `await` на вызовах закрытия скоупа прямо в IDE.
