...
Если ваш класс реализует интерфейс `Disposable`, просто вызовите `scope.dispose()`, когда хотите освободить все ресурсы — CherryPick вызовет `dispose()` для всех объектов, которые реализуют Disposable и были резолвлены из DI.

Это помогает избежать утечек памяти, корректно завершать рабочие процессы и освобождать любые ресурсы (файлы, потоки, соединения и т.д.).
...
class LoggingService implements Disposable {
  @override
  void dispose() {
    // Закрыть файлы, потоки, соединения и т.д.
...
void main() {
  ...
  // Используем logger...
  scope.dispose(); // выведет: LoggingService disposed!
}
