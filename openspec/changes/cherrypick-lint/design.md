...
- [Risk] `custom_lint_builder` фиксирует `analyzer ^8.0.0`, на мажор младше `analyzer ^9.0.0` в `cherrypick_generator` → зафиксировать `^0.8.1` (актуальная на момент реализации; `^0.7.0` тянет `analyzer ^7.0.0`, ещё дальше от `^9.0.0`). Прямого конфликта резолвинга между пакетами нет, поскольку монорепо не использует нативные pub workspaces (нет `workspace:` в корневом `pubspec.yaml`) — каждый пакет резолвится независимо. **Важно:** это не временный разрыв — репозиторий [`invertase/dart_custom_lint`](https://github.com/invertase/dart_custom_lint) заархивирован, релиза `custom_lint_builder` с поддержкой `analyzer` 9.x ожидать не стоит. `cherrypick_lint` останется на `analyzer` 8.x, пока не появится форк проекта или пакет не переедет на другую инфраструктуру линтинга.
...
Это ограничение не временное: репозиторий
[`invertase/dart_custom_lint`](https://github.com/invertase/dart_custom_lint)
заархивирован (проверено через GitHub API), поэтому релиза
`custom_lint_builder` с поддержкой `analyzer` 9.x ожидать не стоит.
`cherrypick_lint` останется на `analyzer` 8.x, пока кто-то не форкнет
проект или пакет не переедет на другую инфраструктуру линтинга — это не
блокирует текущую реализацию (пакеты резолвятся независимо), но стоит
учитывать в будущих решениях по сопровождению `cherrypick_lint`.
