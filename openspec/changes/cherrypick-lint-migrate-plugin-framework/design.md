...
- **Целевые версии не фиксируются одной парой — пакет поддерживает и Flutter 3.38.x, и 3.41+ (см. следующий пункт); запиненный в репозитории SDK — Flutter 3.38.5 (Dart 3.10.4).** Каждая опубликованная версия `analysis_server_plugin` пинует `analyzer` точной версией, а `analyzer` ≥13.1.0 требует SDK `^3.11.0`: под Dart 3.10.x `pub` разрешает максимум 0.3.14/`analyzer` 12.1.0, под Dart 3.11.x — 0.3.22/14.3.0 (проверено реальным `pub get` на обоих SDK).
  История решения: сначала перенос делался на старую пару, чтобы не поднимать нижнюю границу SDK у пользователей; затем пин репозитория поднимался до Flutter 3.41.7 — тогда казалось, что интермиттентный краш analysis server лечится только этим; и наконец пин вернулся на 3.38.5, когда выяснилось, что краш был дефектом конкретно Dart 3.10.0 и на Dart 3.10.4 не воспроизводится. Итог: разработка и CI идут на Flutter 3.38.5, а исходники собираются и тестируются на обоих мажорах `analyzer`.
- **Констрейнты оставлены широкими, чтобы пакет работал и на Flutter 3.38.x, и на 3.41+.** `analysis_server_plugin: ">=0.3.14 <0.4.0"`, `analyzer: ">=12.1.0 <15.0.0"`, `analyzer_plugin: ">=0.14.8 <0.15.0"`, `analyzer_testing: ">=0.2.5 <0.5.0"`, `environment: sdk: ">=3.10.0 <4.0.0"` (3.10 — первый SDK с секцией `plugins:`). Проверено реальным резолвом и прогоном на обоих SDK из одних исходников: под Dart 3.10.0 — 0.3.14/`analyzer` 12.1.0/`analyzer_testing` 0.2.5, под Dart 3.11.5 — 0.3.22/14.3.0/0.4.1; в обоих случаях `dart analyze` по пакету чист и все 55 тестов проходят. Правила и фиксы собираются на двух мажорах `analyzer` без правок, так что дуальная поддержка ничего не стоит в коде.
  Пакет-потребитель под Dart 3.10 получает корректные диагностики стабильно. Отдельная оговорка про Dart 3.10.0: `dart analyze` над `cherrypick_lint/example` — пакетом внутри самого пакета-плагина — там падал примерно раз в три прогона, независимо от свежести кэша плагина и формы `path`. На Dart 3.10.4 (Flutter 3.38.5) это не воспроизводится (6/6 чистых прогонов), поэтому исключать `cherrypick_lint_example` из `melos run analyze` не нужно и репозиторий запинован на 3.38.5.
...
Поверх этого у Dart **3.10.0** обнаружилась своя, независимая от кэша флакучесть:
`dart analyze` над пакетом, лежащим внутри пакета-плагина (наш `example`), падал
там примерно раз в три прогона даже со свежим кэшем. На Dart 3.10.4 (Flutter
3.38.5) она не воспроизводится, на Dart 3.11.5 — тоже. То есть ни то, ни другое
явление не требует апгрейда мажора SDK: репозиторий запинован на Flutter 3.38.5,
`cherrypick_lint` и `cherrypick_lint_example` анализируются наравне со всеми,
