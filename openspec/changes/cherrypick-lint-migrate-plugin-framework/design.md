...
- **Целевые версии — `analysis_server_plugin` 0.3.22 + `analyzer` 14.3.0 + `analyzer_plugin` 0.14.16; запиненный SDK поднят до Flutter 3.41.7 (Dart 3.11.5).** Каждая опубликованная версия `analysis_server_plugin` пинует `analyzer` точной версией, а `analyzer` ≥13.1.0 требует SDK `^3.11.0`: под прежним пином Dart 3.10.0 `pub` разрешал максимум 0.3.14/`analyzer` 12.1.0 (проверено реальным `pub get` на обоих SDK). Сначала перенос делался именно на эту старую пару, чтобы не поднимать нижнюю границу SDK у пользователей; решение изменено после того, как выяснилось, что краш analysis server — дефект самого Dart 3.10.0 и на новом фреймворке воспроизводится тоже (см. отдельный раздел ниже). Оставаться на 3.10.0 значило бы сохранить обходной путь в `melos run analyze`, то есть не получить главного, ради чего затевалась миграция. Цена: `cherrypick_lint` 2.0.0 требует у пользователей Dart ≥3.11 (Flutter ≥3.41) — это фиксируется в `environment: sdk` пакета и в CHANGELOG.
...
## Краш analysis server на Dart 3.10.0 воспроизводится и на новом фреймворке

Ключевое допущение proposal.md — «`analysis_server_plugin` устраняет и сам источник
этой проблемы… отпадает нужда в обходном пути» — **не подтвердилось на Dart 3.10.0**.
`dart analyze` над `cherrypick_lint/example` с плагином на `analysis_server_plugin`
падает с `Bad state: The analysis server crashed unexpectedly` перемежающимся образом
(устойчивая картина: первые два прогона чистые, третий и последующие — краш).

Что проверено, чтобы отделить причину:

| Сценарий | Dart 3.10.0 | Dart 3.11.5 |
|---|---|---|
| Тривиальный плагин-скелет (одно правило на `AwaitExpression`), отдельный пакет-потребитель | 6/6 чисто | — |
| `cherrypick_lint` (13 правил), отдельный пакет-потребитель вне репо | 6/6 чисто | — |
| Каждая фикстура `example/lib/*.dart` по отдельности, в пакете вне репо | чисто | — |
| Все четыре фикстуры вместе, в пакете вне репо | 4/4 чисто | — |
| `cherrypick_lint/example` как есть (в репо) | краш на 3-м прогоне | 4/4 чисто |
| Копия `example` внутри репо, но вне каталога плагина | краш на 3-м прогоне | — |
| `example` в репо + плагин, скопированный вне репо | краш на 3-м прогоне | — |

Выводы:

- Краш **не вызван нашими правилами**: тривиальный плагин и тот же плагин на другом
  пакете-потребителе не падают ни разу.
- Краш **не вызван содержимым фикстур**: те же файлы в пакете вне репо анализируются
  чисто, включая все четыре разом.
- Краш **не вызван вложенностью** `example` внутрь пакета-плагина: он одинаково
  воспроизводится, если плагин лежит вне репо, и если анализируемая копия лежит в
  репо рядом с плагином.
- Краш **исчезает на Dart 3.11.5** (Flutter 3.41.7) на том же коде и тех же
  запиненных версиях `analysis_server_plugin` 0.3.14 / `analyzer` 12.1.0.

То есть это дефект инфраструктуры плагинов в самом Dart 3.10.0 — первом SDK с
поддержкой `plugins:` — а не свойство `custom_lint`, как предполагалось в proposal.md.
Именно эта находка изменила решение по SDK: репозиторий поднят до Flutter 3.41.7
(Dart 3.11.5), где `dart analyze` над `example` чист 5/5 прогонов уже на актуальных
0.3.22/`analyzer` 14.3.0. Задачи 5.5 и 5.6 выполняются в полном объёме — исключение
из `analyze` и шаг `lint:custom_lint` снимаются. Если бы репозиторий остался на 3.10.0,
исключение пришлось бы сохранить, иначе CI краснел бы случайным образом.
