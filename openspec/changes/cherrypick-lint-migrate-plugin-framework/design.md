...
Практическое следствие: пока репозиторий запинован на Flutter 3.38.1, задачи 5.5 и 5.6
(вернуть `cherrypick_lint`/`cherrypick_lint_example` в обычный `melos run analyze` и
убрать шаг `lint:custom_lint` из CI) выполнимы только частично — исключение из
`analyze` придётся сохранить, иначе CI будет краснеть случайным образом.
