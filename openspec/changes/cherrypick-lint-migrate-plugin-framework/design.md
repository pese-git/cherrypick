...
- **Констрейнты оставлены широкими, чтобы пакет работал и на Flutter 3.38.x, и на 3.41+.** `analysis_server_plugin: ">=0.3.14 <0.4.0"`, `analyzer: ">=12.1.0 <15.0.0"`, `analyzer_plugin: ">=0.14.8 <0.15.0"`, `analyzer_testing: ">=0.2.5 <0.5.0"`, `environment: sdk: ">=3.10.0 <4.0.0"` (3.10 — первый SDK с секцией `plugins:`). Проверено реальным резолвом и прогоном на обоих SDK из одних исходников: под Dart 3.10.0 — 0.3.14/`analyzer` 12.1.0/`analyzer_testing` 0.2.5, под Dart 3.11.5 — 0.3.22/14.3.0/0.4.1; в обоих случаях `dart analyze` по пакету чист и все 55 тестов проходят. Правила и фиксы собираются на двух мажорах `analyzer` без правок, так что дуальная поддержка ничего не стоит в коде.
  Пакет-потребитель под Dart 3.10 получает корректные диагностики стабильно (9 прогонов на двух разных потребителях без сбоев). Но `dart analyze` над `cherrypick_lint/example` — пакетом внутри самого пакета-плагина — под Dart 3.10 падает примерно раз в три прогона, независимо от свежести кэша плагина и от того, относительный `path: ../` или абсолютный; под Dart 3.11.5 не падает ни разу. Это отдельное явление от кэша синтетического пакета (см. раздел про краш) и означает, что на Flutter 3.38 пришлось бы либо снова исключать `cherrypick_lint_example` из `melos run analyze`, либо вынести пример из каталога плагина. Поэтому пин репозитория остаётся на Flutter 3.41.7 — разработка и CI идут на нём, а нижняя граница пакета опущена до 3.10 ради пользователей.
